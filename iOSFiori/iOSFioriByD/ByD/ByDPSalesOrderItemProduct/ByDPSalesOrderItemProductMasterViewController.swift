//
// ByDPSalesOrderItemProductMasterViewController.swift
// ByD
//
...
class ByDPSalesOrderItemProductMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var byDC: ByDC<OnlineODataProvider> {
        return self.appDelegate.byDC
    }

    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([ByDPSalesOrderItemProduct]?, Error?) -> Void) -> Void)?
    private var entities: [ByDPSalesOrderItemProduct] = [ByDPSalesOrderItemProduct]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let salesorderitemproduct = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ObjectID", value: "\(salesorderitemproduct.objectID!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! ByDPSalesOrderItemProductDetailViewController
...
extension ByDPSalesOrderItemProductMasterViewController: EntitySetUpdaterDelegate {
