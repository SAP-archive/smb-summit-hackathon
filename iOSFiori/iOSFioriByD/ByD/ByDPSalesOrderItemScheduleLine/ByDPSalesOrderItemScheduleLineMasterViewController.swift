//
// ByDPSalesOrderItemScheduleLineMasterViewController.swift
// ByD
//
...
class ByDPSalesOrderItemScheduleLineMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var byDC: ByDC<OnlineODataProvider> {
        return self.appDelegate.byDC
    }

    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([ByDPSalesOrderItemScheduleLine]?, Error?) -> Void) -> Void)?
    private var entities: [ByDPSalesOrderItemScheduleLine] = [ByDPSalesOrderItemScheduleLine]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let salesorderitemscheduleline = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ObjectID", value: "\(salesorderitemscheduleline.objectID!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! ByDPSalesOrderItemScheduleLineDetailViewController
...
extension ByDPSalesOrderItemScheduleLineMasterViewController: EntitySetUpdaterDelegate {
