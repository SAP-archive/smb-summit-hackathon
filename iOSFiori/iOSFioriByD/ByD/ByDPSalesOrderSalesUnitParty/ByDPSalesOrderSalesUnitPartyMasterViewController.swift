//
// ByDPSalesOrderSalesUnitPartyMasterViewController.swift
// ByD
//
...
class ByDPSalesOrderSalesUnitPartyMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var byDC: ByDC<OnlineODataProvider> {
        return self.appDelegate.byDC
    }

    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([ByDPSalesOrderSalesUnitParty]?, Error?) -> Void) -> Void)?
    private var entities: [ByDPSalesOrderSalesUnitParty] = [ByDPSalesOrderSalesUnitParty]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let salesordersalesunitparty = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ObjectID", value: "\(salesordersalesunitparty.objectID!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! ByDPSalesOrderSalesUnitPartyDetailViewController
...
extension ByDPSalesOrderSalesUnitPartyMasterViewController: EntitySetUpdaterDelegate {
