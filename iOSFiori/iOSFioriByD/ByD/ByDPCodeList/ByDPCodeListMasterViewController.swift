//
// ByDPCodeListMasterViewController.swift
// ByD
//
...
class ByDPCodeListMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var byDC: ByDC<OnlineODataProvider> {
        return self.appDelegate.byDC
    }

    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([ByDPCodeList]?, Error?) -> Void) -> Void)?
    private var entities: [ByDPCodeList] = [ByDPCodeList]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let codelist = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "Code", value: "\(codelist.code!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! ByDPCodeListDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].code ?? ""
...
extension ByDPCodeListMasterViewController: EntitySetUpdaterDelegate {
