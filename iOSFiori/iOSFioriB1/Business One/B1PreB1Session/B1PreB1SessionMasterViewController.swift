//
// B1PreB1SessionMasterViewController.swift
// Business One
//
...
class B1PreB1SessionMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var b1Class: B1Class<OnlineODataProvider> {
        return self.appDelegate.b1Class
    }

    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([B1PreB1Session]?, Error?) -> Void) -> Void)?
    private var entities: [B1PreB1Session] = [B1PreB1Session]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let b1session = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "SessionId", value: "\(b1session.sessionID!)")
        return cell
    }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        self.b1Class.deleteEntity(currentEntity) { error in
            if let error = error {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! B1PreB1SessionDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].sessionID ?? ""
...
extension B1PreB1SessionMasterViewController: EntitySetUpdaterDelegate {
