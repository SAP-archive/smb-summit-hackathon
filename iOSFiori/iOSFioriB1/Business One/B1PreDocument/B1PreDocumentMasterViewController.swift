//
// B1PreDocumentMasterViewController.swift
// Business One
//
...
class B1PreDocumentMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var b1Class: B1Class<OnlineODataProvider> {
        return self.appDelegate.b1Class
    }

    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([B1PreDocument]?, Error?) -> Void) -> Void)?
    private var entities: [B1PreDocument] = [B1PreDocument]()
...
//        return objectCell
        ...
        let document = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "DocEntry", value: "\(document.docEntry!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! B1PreDocumentDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].docEntry != nil ? "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].docEntry!)" : "")"
...
extension B1PreDocumentMasterViewController: EntitySetUpdaterDelegate {
